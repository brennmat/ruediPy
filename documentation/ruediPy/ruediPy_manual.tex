...
\texttt{sudo pip3 -H install pydigitemp}\\
...
You should also consider setting up the computer to avoid going to `sleep' mode, because this might interrupt the measurement procedure. Finally, it may be useful to increase the size of the cursor if it's hard to see on the screen during poor light conditions during field work. This can be achieved using the following command (for the GNOME desktop environment):\par
