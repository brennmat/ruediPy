# Class for pressure sensors running via an Arduino
#
# DISCLAIMER:
...
# Copyright (C) 2024  Matthias Brennwald
# 
...
#
# Copyright 2016, 2017, 2018, 2024 Matthias Brennwald (brennmat@gmail.com)
...
class pressuresensor_ARDUINO:
	"""
	ruediPy class for pressure sensors via an Arduino
	"""
	...
	def __init__( self , serialport , baudrate = 115200, label = 'PRESSURESENSOR' , plot_title = None , max_buffer_points = 500 , fig_w = 6.5 , fig_h = 5 , has_plot_window = True , has_external_plot_window = None, P_unit = 'bar'):
		'''
		pressuresensor_ARDUINO.__init__( serialport , label = 'PRESSURESENSOR' , plot_title = None , max_buffer_points = 500 , fig_w = 6.5 , fig_h = 5 , has_plot_window = True , has_external_plot_window = None, P_unit = 'bar' )
		
		Initialize PRESSURESENSOR object (ARDUINO), configure serial port connection
		
		INPUT:
		serialport: device name of the serial port, e.g. serialport = '/dev/ttyUSB3'
		baudrate: baud rate for use with serial connection
		...
		if P_unit.upper() == 'HPA':
		    ...
		    self.warning( 'Could not initialize ARDUINO pressure sensor: unit ' + P_unit + ' is not supported.')
	...
	def get_serial_lock(self):
		'''
		pressuresensor_ARDUINO._get_serial_lock()
		
		Lock serial port for exclusive access (important if different threads / processes are trying to use the port). Make sure to release the lock after using the port (see pressuresensor_ARDUINO._release_serial_lock()!
	...
	def release_serial_lock(self):
		'''
		pressuresensor_ARDUINO._release_serial_lock()
	...
	def label(self):
		"""
		label = pressuresensor_ARDUINO.label()
	...
	def pressure(self,f,add_to_pressbuffer=True):
		"""
		press,unit = pressuresensor_ARDUINO.pressure(f,add_to_pressbuffer=True)
		...
		if not(hasattr(self,'ser')):
			...
			try:
				...
				self.release_serial_lock()
				self.warning( 'An unknown error occured while reading the ARDUINO pressure sensor!' )

		# write data to datafile
		if not ( f == 'nofile' ):
			f.write_pressure('PRESSURESENSOR_ARDUINO',self.label(),p,unit,t)
	...
	def warning(self,msg):
		'''
		pressuresensor_ARDUINO.warning(msg)
	...
	def pressbuffer_add(self,t,p,unit):
		"""
		pressuresensor_ARDUINO.pressbuffer_add(t,p,unit)
	...
	def pressbuffer_get_timestamp(self):
		"""
		timestamp = pressuresensor_ARDUINO.pressbuffer_get_timestamp()
	...
	def plot_pressbuffer(self):
		'''
		pressuresensor_ARDUINO.plot_pressbuffer()
	...
	def pressbuffer_clear(self):
		"""
		pressuresensor_ARDUINO.pressbuffer_clear()
